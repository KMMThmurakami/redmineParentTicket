javascript:(function() {
  ...
  function bmlPanel(id, w, h) {
    ...
    // コンテンツ要素のスタイルを設定
    with (F.style) {
      (height = h - 20 - 4 + 'px'), (backgroundColor = '#222'), (color = '#eee'), (fontSize = '11px'), (cursor = 'auto');
    }
    ...
    // クローズボタン要素の属性とスタイルを設定
		(CL.title = 'close');
    // クローズボタンのスタイルを設定
		CL.style.cssText = 'position: absolute; top: 5px; left: 5px; height: 10px; width: 10px; cursor: pointer; text-align: center; line-height: 10px; background-color: red; color: #fff;';
  ...
  SP.content.innerHTML = '<div style="padding: 10px;">テキスト入力欄にリリース日を mm/dd の形式で入力してください</div>';
  ...
  var textNode = D.createTextNode('作成したいチケットのシーズンを選択してください');
  ...
  // 検索エンジンのラジオボタンとラベルを作成し、パネルに追加
    ...
    var container = D.createElement('div');
    container.style.cssText = 'display: flex; align-items: center; margin: 6px;';
    C.appendChild(container);
  ...
  with (sbm) {
    (innerHTML = '選択したサービスで検索'),
      (style.cssText = 'cursor:pointer; margin:6px auto; display:block; clear:both; padding: 6px 12px; background-color: #4CAF50; color: white; border: none; border-radius: 4px; font-size: 14px;');
